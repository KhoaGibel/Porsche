...
import * as graphql_star from '@lens-protocol/graphql';
import { Role, ChallengeMutation, AuthenticateMutation, RefreshMutation, RevokeAuthenticationMutation, SwitchAccountMutation, TransactionStatusQuery } from '@lens-protocol/graphql';
import { schema } from '@lens-protocol/graphql/test-utils';
import * as types_star from '@lens-protocol/types';
import { evmAddress, hexString, ResultAwareError, err, ok, never as never$1, okAsync, ResultAsync, errAsync, signatureFrom, uuid, invariant as invariant$1, Deferred } from '@lens-protocol/types';
import { validate, visit, Kind, OperationTypeNode } from 'graphql';
import { createWalletClient, http, createPublicClient as createPublicClient$1 } from 'viem';
import { privateKeyToAccount } from 'viem/accounts';
import * as env_star from '@lens-protocol/env';
import { jwtDecode } from 'jwt-decode';
import introspectedSchema from '@lens-protocol/graphql/schema';
import { gql, fetchExchange, createClient } from '@urql/core';
...
__reExport(src_exports, env_star);
__reExport(src_exports, graphql_star);
__reExport(src_exports, types_star);
...
var UnauthenticatedError = class _UnauthenticatedError extends ResultAwareError {
  name = "UnauthenticatedError";
  static fromCombinedError(error) {
    return new _UnauthenticatedError(error.message, { cause: error });
  }
};
var UnexpectedError = class extends ResultAwareError {
  name = "UnexpectedError";
};
var AuthenticationError = class extends ResultAwareError {
  name = "AuthenticationError";
};
var SigningError = class extends ResultAwareError {
  name = "SigningError";
};
var TransactionError = class extends ResultAwareError {
  name = "TransactionError";
};
var TransactionIndexingError = class extends ResultAwareError {
  name = "TransactionIndexingError";
};
var MetadataIndexingError = class extends ResultAwareError {
...
function authenticatedUser(claims) {
  switch (claims[ROLE_CLAIM]) {
    case Role.AccountManager:
      return ok({
        address: claims.act?.sub ?? never$1("Account Manager must have an Actor Claim"),
        app: claims.aud,
    ...
    case Role.AccountOwner:
      return ok({
        address: claims.act?.sub ?? never$1("Account Owner must have an Actor Claim"),
        app: claims.aud,
        authenticationId: claims.sid,
...
var cache = /* @__PURE__ */ cacheExchange({
...
var BatchQueryBuilder = class {
  ...
  addQuery = (document, variables) => {
    ...
    return ResultAsync.fromPromise(deferred.promise, (err3) => {
  ...
  build() {
    ...
    for (const { alias, document, variables } of this.queries) {
      ...
      const localDefs = operation.variableDefinitions?.map((v) => {
        ...
        mergedVariables[newVarName] = variables[v.variable.name.value] ?? never$1();
        return {
          ...v,
          variable: {
            ...v.variable,
  ...
  for (const definition of document.definitions) {
    switch (definition.kind) {
      ...
      case Kind.FRAGMENT_DEFINITION:
        ...
        break;
      default:
        never$1(`Unexpected definition kind: ${definition.kind}`);
    }
  }
  invariant$1(operation, "No operation definition found in the document");
...
function takeValue({
  ...
  invariant$1(data, `Expected a value, got: ${error?.message}`);
  return data.value;
}
var AbstractClient = class {
  ...
  resultFrom(source) {
    return ResultAsync.fromPromise(source.toPromise(), (err3) => {
    ...
    const constantExchanges = [
      authExchange(async (utils) => {
        return {
          ...
          refreshAuth: async () => {
            const credentials = this.getCredentials().unwrapOr(null) ?? never$1("Missing refresh token");
...
// src/test-utils.ts
var CHAIN = chains.testnet;
var TEST_ACCOUNT = evmAddress(import.meta.env.TEST_ACCOUNT);
var TEST_APP = evmAddress(import.meta.env.TEST_APP);
var TEST_ERC20 = evmAddress(import.meta.env.TEST_ERC20);
var PRIVATE_KEY = hexString(import.meta.env.PRIVATE_KEY);
var GLOBAL_SPONSORSHIP = evmAddress(
  import.meta.env.GLOBAL_SPONSORSHIP
);
var SPONSORSHIP_APPROVER_PRIVATE_KEY = hexString(
  import.meta.env.SPONSORSHIP_APPROVER_PRIVATE_KEY
);
var signer = privateKeyToAccount(import.meta.env.PRIVATE_KEY);
var environment = import.meta.env.ENVIRONMENT === "local" ? src_exports.local : import.meta.env.ENVIRONMENT === "staging" ? src_exports.staging : src_exports.testnet;
var wallet = createWalletClient({
  account: signer,
  chain: CHAIN,
  transport: http()
});
var TEST_SIGNER = evmAddress(signer.address);
...
function assertTypedDocumentSatisfies(document, rules) {
  globalExpect(validate(schema, document, rules)).toEqual([]);
}
...
export { CHAIN, GLOBAL_SPONSORSHIP, PRIVATE_KEY, SPONSORSHIP_APPROVER_PRIVATE_KEY, TEST_ACCOUNT, TEST_APP, TEST_ERC20, TEST_SIGNER, assertTypedDocumentSatisfies, createGraphQLErrorObject, createPublicClient, environment, loginAsAccountOwner, loginAsBuilder, loginAsOnboardingUser, mockAccessToken, signer, storageClient, updateTextOnlyMetadata, uploadImmutableTextOnlyPostMetadata, uploadTextOnlyPostMetadata, waitForTransactionReceipt, wallet };
